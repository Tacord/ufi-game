...
func _process(delta: float) -> void:
	...
	if abs(global_position.x - magnet.global_position.x) < 500 and abs(global_position.y - magnet.global_position.y) < 400:
		var direction: Vector2 = global_position.direction_to(magnet.global_position)
		var movement_amount: Vector2 = direction * 500 * delta
